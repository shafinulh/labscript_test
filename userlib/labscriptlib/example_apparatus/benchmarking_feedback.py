...
'''
Define the feedback function that runs at the end of a shot
'''
def compute_feedback(shot_context,t):
    # note: typical usage probably includes the execution of a feedback routine
    # it could also update a certain value
    # but we will implement a dummy example: check what is the peak value of the 
    # fluoresence signal (AI1) if it is not 3 yet, then multiply the analog output 
    # ramp peak value by 1.2
    ...
    if ai1_signal[1].max() < 3:        
        ...
        rm.set_globals({'ramp_vf':new_ramp_vf})
        rm.engage()

feedback.add_function(t='stop',function=compute_feedback)
